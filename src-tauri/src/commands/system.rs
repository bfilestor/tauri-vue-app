...
#[tauri::command]
pub async fn reset_checkup_data(
    db: State<'_, Database>,
    ...
    conn.execute("DELETE FROM ai_analyses", [])
        .map_err(|e| e.to_string())?;
    conn.execute("DELETE FROM checkup_files", [])
        .map_err(|e| e.to_string())?;
    conn.execute("DELETE FROM checkup_records", [])
        .map_err(|e| e.to_string())?;
...
pub async fn reset_all_data(
    ...
    conn.execute("DELETE FROM indicators", [])
        .map_err(|e| e.to_string())?;
    conn.execute("DELETE FROM checkup_projects", [])
        .map_err(|e| e.to_string())?;
...
pub async fn backup_data(
    ...
    let result = (|| -> Result<(), String> {
        // 2 Create zip file
        let file =
            fs::File::create(&target_path).map_err(|e| format!("创建备份文件失败: {}", e))?;
        ...
        if db_path.exists() {
            zip.start_file("health_guard.db", options)
                .map_err(|e| e.to_string())?;
